...
data_dir = # path to original training dataset
...
csv_logger = CSVLogger('training_log_orig.csv', append=True)
...
for activation in activations:
    for optimizer in optimizers:
        ...
        with open('training_log_orig.csv', 'a') as f:
        ...
        model.save(f'models/inceptionv3_orig_{activation}_{optimizer}.keras', overwrite=True)

        test_history = model.evaluate(test_generator, return_dict=True)

        with open('testing_log_orig.csv', 'a') as f:
            f.write(f'InceptionV3 - activation: {activation} optimizer: {optimizer}\n')
