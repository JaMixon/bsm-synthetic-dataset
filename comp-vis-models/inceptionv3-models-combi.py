...
data_dir = # path to combined training dataset
test_data_dir = # path to test dataset

batch_size = 32
img_height = 229 # default InceptionV3 input size
img_width = 229
...
csv_logger = CSVLogger('training_log_combi.csv', append=True)
...
for activation in activations:
    for optimizer in optimizers:
        ...
        with open('training_log_combi.csv', 'a') as f:
            f.write(f'InceptionV3 - activation: {activation} optimizer: {optimizer}\n')
        ...
        model.save(f'models/inceptionv3_combi_{activation}_{optimizer}.keras', overwrite=True)

        test_history = model.evaluate(test_generator, return_dict=True)

        with open('testing_log_combi.csv', 'a') as f:
            f.write(f'InceptionV3 - activation: {activation} optimizer: {optimizer}\n')
